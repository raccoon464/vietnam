...
####################################
# pk = Приватный ключ
# me = Адрес отправителя
# to = Адрес получателя
# amount = Сумма
# gas = Комиссия в газах (требует более глубоких знаний в БЧ)
####################################
...
def transfer_usdt(pk, me, to, amount, gas = 0):
    w3 = Web3(Web3.HTTPProvider(URL))
...
def get_balance_usdt(addres):
    w3 = Web3(Web3.HTTPProvider(URL))
    contract = w3.eth.contract(address=CONTRACT, abi=ABI)
    balanceOf = contract.functions.balanceOf(addres).call()
    return w3.fromWei(balanceOf, 'ether')

def get_balance_bnb(addres):
    w3 = Web3(Web3.HTTPProvider(URL))
    balanceOf = w3.eth.getBalance(addres)
    return w3.fromWei(balanceOf, 'ether')

def gas_price():
    r = requests.get('https://api.bscscan.com/api?module=gastracker&action=gasoracle&apikey=' + YOUR_API_KEY + '')
    r.json()
